...
{
	public:
	ForceLand() : Node("force_land"), need_land(false), landing_triggered(false), is_landed(true)
	{
		...
		land_detected_subscription_ = this->create_subscription<px4_msgs::msg::VehicleLandDetected>("/fmu/out/vehicle_land_detected",
	...
	bool need_land;
	bool landing_triggered;  // Flag to track if landing has been triggered
	bool is_landed;          // Flag to track if vehicle is currently landed

	void land_detected_callback(const px4_msgs::msg::VehicleLandDetected::UniquePtr msg)
	{
		is_landed = msg->landed;
		std::cout << "[DEBUG] is_landed: " << (is_landed ? "true" : "false") 
		          << ", landing_triggered: " << (landing_triggered ? "true" : "false") << std::endl;
		
		// Reset the landing_triggered flag only when vehicle has completed landing
		if(is_landed && landing_triggered)
		{
			std::cout << "Landing completed. System reset." << std::endl;
	...
	void height_callback(const px4_msgs::msg::VehicleLocalPosition::UniquePtr msg) 
	{
		...
		// 1. Height exceeds threshold
		// 2. Landing has not been previously triggered OR landing was completed
		if(z_ > 20 && !landing_triggered)
		{
			need_land = true;
		}
	...
	void activate_switch()
	{
		if(need_land)
		{
			...
			need_land = false;
			landing_triggered = true;  // Mark that landing has been triggered
			std::cout << "Landing procedure triggered. Cannot be re-triggered until landing is completed." << std::endl;
		}
	}
};
